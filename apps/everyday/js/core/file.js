...
export const FileHelper = {
  ...
  validateImage(file, maxMB = LIMITS.IMAGE_MB) {
    ...
    const hasKnownMime = IMAGE_MIMES.includes(String(file.type || '').toLowerCase());
    const hasKnownExt = IMAGE_EXTS.includes(fileExt);
    if (file.type && (!file.type.startsWith(IMAGE_MIMES_PREFIX) || !hasKnownMime) && !hasKnownExt) {
      return { ok: false, error: 'That doesn\'t look like an image.' };
    }
    if (!hasKnownExt && !hasKnownMime) {
      return { ok: false, error: 'Unsupported image format. Use JPG, PNG, WebP, GIF, BMP, AVIF, HEIC, or HEIF.' };
    }
    return { ok: true };
  },
