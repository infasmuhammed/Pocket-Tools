...
function initInstallPrompt() {
  ...
  if (!btn || window.matchMedia('(display-mode: standalone)').matches) return;
...
function pushRecent(id) {
  ...
  list.unshift(id);
  SafeStorage.setItem(RECENT_KEY, JSON.stringify(list.slice(0, RECENT_MAX)));
}
...
function restoreHomePosition() {
  ...
  window.scrollTo(0, Number(SafeSessionStorage.getItem(HOME_SCROLL_KEY) || 0));
}
